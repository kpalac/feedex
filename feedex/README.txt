...
Features:
  ...
  - Desktop clipboard integration with selected text and window names. They can be used for manually adding entries, e.g. by keyboard shortcut.
  ...
  - Support for queries:
      ...
      c) Term contexts - display contexts in which a term appears
      d) Similarity search - find document similar to one selected
      e) Time series - generate time series for a term, choose grouping (can generate a plot)
      f) Document's relevance in time - generate time series of document's keywords (can generate a plot)
      g) Term relatedness - which terms go together
      h) Grouping by category, channel or flag with preferred depth, allowing for nice news summaries in a form of a tree
      h.1) Grouping by similarity, i.e. collapsing similar articles into a best choice to compact long lists
      h.2) Grouping by months, days or hours
  ...
  - Keyword extractiom is based on language models - no fancy stuff here - just phrase lists and extraction rules
    ...
    See smallsem.py module for more information on how basic NLP is done by Feedex 
