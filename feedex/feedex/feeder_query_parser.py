...
{scast(r[self.result.get_index(f)], str, '')}"""
        ...
        if kargs.get('print',self.print):
            self.cli_table_print([n_("Keyword"), n_("Frequency")], self.results, output='csv', html_cols=('keyword','frequency',))
    ...
    def relevance_in_time(self, id, **kargs):
        ...
        if kargs.get('print',self.print) and not plot:
            self.cli_table_print([col_name, n_("Frequency")], time_series, output='csv', html_cols=('time','frequency',))
    ...
    def rules_for_entry(self, id:int, **kargs):
        ...
        importance, flag, best_entries, flag_dist, results = entry.ling(index=False, rank=True, to_var=True)
...
#
    ...
    def list_flags(self, **kargs):
        ...
        if kargs.get("print",self.print):
            self.cli_table_print(FLAGS_SQL_TABLE_PRINT, self.results, number_footnote='flags', flag_col=0, html_cols=FLAGS_SQL_TABLE) 
        else: return self.results
    ...
    def read_feed(self, id:int, **kargs):
        ...
        if self.output in ('csv','json','html'):
            self.cli_table_print(FEEDS_SQL_TABLE + ('parent_category_name',), (lfield,), html_cols=FEEDS_SQL_TABLE + ('parent_category_name',))
            return lfield
        else:
    ...
    def read_entry(self, id:int, **kargs):
        ...
        if self.output in ('json','html','csv'):
            self.cli_table_print(RESULTS_SQL_TABLE, (lentry,), html_cols=RESULTS_SQL_TABLE)
            return lentry
        
        if lentry in ((), None, [], (None,)): return ''

        lrules = self.FX.qr_sql("""select name from rules r where r.context_id = :id""", {'id':id} , all=True, ignore_errors=False) 
        if self.FX.db_error is not None: return ''
        
        if self.debug in (1,8):
            lterms = []
            try: ix_doc = self.FX.ix.get_document(lentry[self.result.get_index('ix_id')])
            except (xapian.DatabaseError, xapian.DocNotFoundError) as e: ix_doc = None
            if isinstance(ix_doc, xapian.Document):
                for t in ix_doc.termlist(): lterms.append(t.term.decode('utf-8')) 
        else: lterms = []
        ...
        if self.debug in (1,8) or kargs.get('to_var',False):
            ...
            if lterms is not None and lrules != []:
                term_str = "-------------------------------------------------------------------------------------------------------------------\n"
                for t in lterms: term_str = f'{term_str}{t} '
                ostring = f"""{ostring}\n{term_str}"""
            if kargs.get('to_var',False):
                return ostring
            print(ostring)
        else:
            self.cli_entry_print(lentry, lrules, lterms)
...
#
    ...
    def cli_entry_print(self, lentry:list, lrules:list, lterms:list):
...
{_('Weight')}:         {entry['weight']}       {_('Importance')}:     {entry['importance']}
    """
        ...
        if lrules is not None and lrules != []:
            ...
            rule_str = ""
            for r in lrules: rule_str = f"""{rule_str}{slist(r,0,'')};  """
            print(rule_str, "\n\n")
