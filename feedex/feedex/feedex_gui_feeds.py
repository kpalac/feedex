...
Hit <b>Ctrl-F</b> for interactive search""") )
    ...
    def copy_feed(self, *args, **kargs):
        ...
        self.parent.update_status(0,(0, _('%a selected for moving...'), self.copy_selected.name()))
    ...
    def reload_feeds(self, *args, **kargs):
        ...
        if self.parent.curr_place == FX_PLACE_LAST: 
            new_row = self.feed_store_tmp.append(None, (self.parent.icons.get('new'), -1, f'<b><u>{_("New")}</u></b>', '', None, FX_PLACE_LAST, 0, None))        
        else: 
            new_row = self.feed_store_tmp.append(None, (self.parent.icons.get('new'), -1, _('New'), '', None, FX_PLACE_LAST, 0, None))

        if self.parent.curr_place == FX_PLACE_PREV_LAST: 
            self.feed_store_tmp.append(new_row, (self.parent.icons.get('calendar'), -7, f'<b><u>{_("Previous Update")}</u></b>', '', None, FX_PLACE_PREV_LAST, 0, None))
        else: 
            self.feed_store_tmp.append(new_row, (self.parent.icons.get('calendar'), -7, _('Previous Update'), '', None, FX_PLACE_PREV_LAST, 0, None))

        if self.parent.curr_place == FX_PLACE_LAST_HOUR: 
            self.feed_store_tmp.append(new_row, (self.parent.icons.get('calendar'), -2, f'<b><u>{_("Last Hour")}</u></b>', '', None, FX_PLACE_LAST_HOUR, 0, None))
        else: 
            self.feed_store_tmp.append(new_row, (self.parent.icons.get('calendar'), -2, _('Last Hour'), '', None, FX_PLACE_LAST_HOUR, 0, None))

        if self.parent.curr_place == FX_PLACE_TODAY: 
            self.feed_store_tmp.append(new_row, (self.parent.icons.get('calendar'), -3, f'<b><u>{_("Today")}</u></b>', '', None, FX_PLACE_TODAY, 0, None))
        else: 
            self.feed_store_tmp.append(new_row, (self.parent.icons.get('calendar'), -3, _('Today'), '', None, FX_PLACE_TODAY, 0, None))
 
        if self.parent.curr_place == FX_PLACE_LAST_WEEK: 
            self.feed_store_tmp.append(new_row, (self.parent.icons.get('calendar'), -4, f'<b><u>{_("This Week")}</u></b>', '', None, FX_PLACE_LAST_WEEK, 0, None))
        else: 
            self.feed_store_tmp.append(new_row, (self.parent.icons.get('calendar'), -4, _('This Week'), '', None, FX_PLACE_LAST_WEEK, 0, None))
 
        if self.parent.curr_place == FX_PLACE_LAST_MONTH: 
            self.feed_store_tmp.append(new_row, (self.parent.icons.get('calendar'), -5, f'<b><u>{_("This Month")}</u></b>', '', None, FX_PLACE_LAST_MONTH, 0, None))
        else: 
            self.feed_store_tmp.append(new_row, (self.parent.icons.get('calendar'), -5, _('This Month'), '', None, FX_PLACE_LAST_MONTH, 0, None))
 
        if self.parent.curr_place == FX_PLACE_LAST_QUARTER: 
            self.feed_store_tmp.append(new_row, (self.parent.icons.get('calendar'), -6, f'<b><u>{_("This Quarter")}</u></b>', '', None, FX_PLACE_LAST_QUARTER, 0, None))
        else: 
            self.feed_store_tmp.append(new_row, (self.parent.icons.get('calendar'), -6, _('This Quarter'), '', None, FX_PLACE_LAST_QUARTER, 0, None))
        ...
        rss_row = self.feed_store_tmp.append(None, (self.parent.icons.get('rss'), -20, _('RSS'), '', None, FX_PLACE_ALL_CHANNELS, 0, None))
        for f in self.parent.FX.MC.feeds:
            self.feed.populate(f)
        ...
        html_row = self.feed_store_tmp.append(None, (self.parent.icons.get('www'), -21, _('HTML'), '', None, FX_PLACE_ALL_CHANNELS, 0, None))
        for f in self.parent.FX.MC.feeds:
            self.feed.populate(f)
        ...
        script_row = self.feed_store_tmp.append(None, (self.parent.icons.get('script'), -22, _('Script'), '', None, FX_PLACE_ALL_CHANNELS, 0, None))
        for f in self.parent.FX.MC.feeds:
            self.feed.populate(f)
        ...
        local_row = self.feed_store_tmp.append(None, (self.parent.icons.get('local'), -23, _('Local'), '', None, FX_PLACE_ALL_CHANNELS, 0, None))
        for f in self.parent.FX.MC.feeds:
            self.feed.populate(f)
        ...
        if self.parent.curr_place == 'trash_bin': deleted_row = self.feed_store_tmp.append(None, (self.parent.icons.get('trash', None), -30, f'<b>{_("Trash")}</b>', '', None, FX_PLACE_TRASH_BIN, 1, None))            
        else: deleted_row = self.feed_store_tmp.append(None, (self.parent.icons.get('trash', None), -30, _('Trash'), '', None, FX_PLACE_TRASH_BIN, 1, None)) 
    ...
    def on_activate_feed(self, *args):
        ...
        if self.selected_place == FX_PLACE_ALL_CHANNELS:
            ...
            for i in range(self.parent.upper_notebook.get_n_pages()):
                tab = self.parent.upper_notebook.get_nth_page(i)
                if tab is None: continue

                if tab.type == FX_TAB_PLACES:
                    tab.query(self.selected_place, {})
                    self.parent.upper_notebook.set_current_page(i)
                    break
            ...
            if self.parent.curr_upper.type in (FX_TAB_PLACES, FX_TAB_SEARCH, FX_TAB_CONTEXTS, FX_TAB_SIMILAR, FX_TAB_TREE, FX_TAB_NOTES, FX_TAB_TRENDING,):
                ...
                if self.selected_feed_id == self.parent.curr_upper.feed_filter_id:
                    ...
                    self.parent.curr_upper.create_list(False, self.selected_feed_id, False )
                    self.parent.curr_upper.feed_filter_id = self.selected_feed_id
