...
If no grouping is selected, it will simply show top results"""))
        ...
        self.ignore_media_button = Gtk.CheckButton.new_with_label(_('Ignore Audio/Video?'))
        self.ignore_media_button.set_tooltip_markup(_('Should audio/video streaming be ignored alltogether? Ueful for better performance'))
...
"""))
        ...
        interface_grid.attach(self.ignore_media_button, 7,4, 5,1)
        ...
        system_grid.attach(win_name_excl_label, 1,10, 4,1)
        system_grid.attach(self.win_name_excl_entry, 6,10, 20,1)
    ...
    def on_restore(self, *args):
        ...
        if self.config.get('ignore_media',False): self.ignore_media_button.set_active(True)
        else: self.ignore_media_button.set_active(False)
    ...
    def get_data(self, *args):
        ...
        if self.ignore_media_button.get_active(): self.result['ignore_media'] = True
        else: self.result['ignore_media'] = False
