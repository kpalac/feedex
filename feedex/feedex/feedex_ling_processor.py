...
class LingProcessor:
    ...
    def __init__(self, top_parent, **kargs):
        ...
        if self.MC.models == {}:
            ...
            if os.path.isdir(FEEDEX_MODELS_PATH):
                for header_file in os.listdir(FEEDEX_MODELS_PATH):
                    ...
                    if header_file.endswith('_header.pkl') and os.path.isfile(header_file):
                        try:
                            ...
                            sys.stderr.write(f'{TERM_ERR}{_("Could not load pickle")} {TERM_ERR_BOLD}{header_file}{TERM_ERR} ...{TERM_NORMAL}\n{e}')
                            continue
                        
                        if self.debug in (1,6,4): print(f'{_("Pickle")} {header_file} {_("loaded")}...')
                ...
                sys.stderr.write(f'{TERM_ERR}{_("Invalid path for language models!")} ({TERM_ERR_BOLD}{FEEDEX_MODELS_PATH}{TERM_ERR}){TERM_NORMAL}\n')
            ...
            try:
                with open(pickle_path, 'rb') as f:
                    ...
                    if type(lst) is not dict: 
                        sys.stderr.write(f'{TERM_ERR}{_("Data not a dictionary structure!")} {TERM_ERR_BOLD}{pickle_path}{TERM_NORMAL}\n')
                        return {}
            except OSError as e:
                sys.stderr.write(f'{TERM_ERR}{_("Could not load pickle")} {TERM_ERR_BOLD}{pickle_path}{TERM_ERR}...{TERM_NORMAL}\n{e}')
                lst = {}
            if self.debug in (1,6,4): print(f'{_("Pickle ")}{pkl} {_("loaded")}...')
        else:
            sys.stderr.write(f'{TERM_ERR}{_("Could not find pickle path")} {TERM_ERR_BOLD}{pickle_path}{TERM_ERR} ...{TERM_NORMAL}\n')
            lst = {}
    ...
    def _extract_patterns(self, field, **kargs):
        ...
        if self.debug in (1,4): print(f'Extracting features from field {str(field)}...')
    ...
    def process_entry(self, entry, **kargs):
        ...
        if index or learn or stats:
            ...
            if self.debug in (1,4) and learn: print(self.features)
    ...
    def detect_lang(self):
        ...
        if self.debug in (1,4): print("Language not provided. Running detection...")
        ...
        if self.debug in (1,4): print("Distribution: ", langs)
    ...
    def match_rules(self, **kargs):
        ...
        if flag_dist != {}: flag = max(flag_dist, key=flag_dist.get)

        if to_var: return importance, flag, best_contexts, flag_dist, display_list
    ...
    def match_fields(self, phrase:dict, qtype:int, field:str, **kargs):
        ...
        if qtype == 0:
            ...
            for f in field_lst:
                ...
                if snippets: 
                    (new_matches, sn) = self.str_matcher(phrase['str_matching'], phrase['str_matching_len'], phrase['beg'], phrase['end'], fs, snippets=True, orig_field=f)
                    for s in sn: snips.append(s)
                else: (new_matches, sn) = self.str_matcher(phrase['str_matching'], phrase['str_matching_len'], phrase['beg'], phrase['end'], fs, snippets=False)
    ...
    def f_phrase(self, string:str, **kargs):
        ...
        def _cond_wc(string):
            ...
            if qtype == 0:
                ...
                if '*' in string or '.' in string: phrase['has_wildcards'] = True
        ...
        if qtype == 0:
            ...
            if regexify:
                ...
                phrase['str_matching_len'] = len(phrase['str_matching'])
            ...
            for t in qtokens:
                ...
                if regexify:
                    tmp_str = f" .......{self._prefix(field, wildcard='.', case_ins=case_ins, case=case)}{tok}"
    ...
    def sregex_matcher(self, string:str, field:str):
