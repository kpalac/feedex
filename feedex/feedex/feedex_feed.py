...
class SQLContainerEditable(SQLContainer):
    ...
    def set_interface(self, interface):
        ...
        else: raise FeedexTypeError(_('Interface should be an instance of Feeder class!'))
    ...
    def add_to_update(self, idict:dict, **kargs):
        ...
        for f, v in idict.items():
            ...
            if (f == 'id' and not allow_id) or f in self.immutable: 
                self.updating = False
                self.restore()
                return -1, _('Editting field %a is not permitted!'), f
    ...
    def constr_update(self, **kargs):
        ...
        if len(self.to_update) == 0: return 1, _('No changes detected')
...
class FeedContainer(FeedContainerBasic):
    ...
    def get_feed_by_id(self, id:int):
        ...
        if feed == -1: 
            self.exists = False
            cli_msg( (-1, _('Channel %a not found!'), id) )
            return -1
        else:
            self.exists = True
            self.populate(feed)
    ...
    def get_cat_by_id(self, id:int):     
        ...
        if feed == -1:
            self.exists = False
            cli_msg( (-1, _('Category %a not found!'), id) )
            return -1
        else:
            self.exists = True
            self.populate(feed)
    ...
    def get_by_id(self, id:int):
        ...
        cli_msg( (-1, _('Channel/Category %a not found!'), id) )
        return -1
    ...
    def r_open(self, **kargs):
        """ Open in browser """
        if not self.exists: return -8, _('Nothing to open!')
        if self.vals['is_category'] == 1: return -7, _('Cannot open a category!')

        if self.vals.get('link') is not None:
            err = ext_open(self.config, 'browser', self.vals.get('link',''), debug=self.debug)
        else: return -8, _('Empty URL!')

        if err == 0: return 0, _('Sent to browser (%a)'), self.vals.get('link', _('<UNKNOWN>'))
        else: return err
    ...
    def validate_regexes(self, **kargs):
        ...
        for f in FEEDS_REGEX_HTML_PARSERS:
            if self.vals[f] is not None and not check_if_regex(self.vals[f]):
                return -7, f'{_("HTML parser: Not a valid REGEX")} ({f}): %a', self.vals[f]
        return 0
    ...
    def validate_regexes2(self, **kargs):
        ...
        for f in ('rx_images','rx_link'):
            if self.vals[f] is not None and not check_if_regex(self.vals[f]):
                return -7, f'{_("Custom entity parser: Not a valid REGEX")} ({f}): %a', self.vals[f]
        return 0
    ...
    def validate(self, **kargs):
        ...
        if self.vals['is_category'] == 1:
            if self.vals['parent_id'] not in (None, 0) and self.vals['parent_id'] == self.vals['id']: return -7, _('Nested categories are not allowed!')
            if self.vals.get('parent_category') not in (None, 0): return -7, _('Nested categories are not allowed!')
        else:
            if self.vals['handler'] in ('rss','html') and not check_url(self.vals['url']): return -7, _('Not a valid URL! (%a)'), self.vals['url']
            if self.vals['link'] is not None and not check_url(self.vals['link']): return -7, _('Not a valid URL! (%a)'), self.vals['link']
            ...
            if self.vals['handler'] not in ('rss','local','html','script'): return -7, _('Invalid handler! Must be rss, html, script or local')
            if self.vals['interval'] is None or self.vals['interval'] < 0: return -7, _('Interval must be >= 0!')
            if self.vals['autoupdate'] not in (None, 0, 1): return -7, _('Autoupdate flag must be 0 or 1!')
            if self.vals['fetch'] not in (None, 0, 1): return -7, _('Fetch flag must be 0 or 1!')

            if self.vals['auth'] is not None:
                if self.vals['auth'] not in ('detect','basic','digest'): return -7, _('Invalid authentication method (must be NONE, detect, basic or digest)')
                if self.vals['passwd'] in ('',None): return -7, _('Password must be provided!')
                if self.vals['login'] in ('',None): return -7, _('Login must be provided!')
            ...
            if 'parent_category' in self.vals.keys():
                if self.vals.get('parent_category') is not None:
                    ...
                    if feed == -1: return -7, _('Category %a not found!'), self.vals['parent_category']
                ...
                if self.vals['parent_id'] is not None:
                    ...
                    if feed == -1: return -7, _('Category %a not found!'), self.vals['parent_id']
                    self.parent_feed.populate(feed)
                    self.vals['parent_id'] = self.parent_feed['id']
                else: self.vals['parent_id'] = None
    ...
    def r_do_update(self, **kargs):
        ...
        if self.FX.rowcount > 0:
            ...
            if not self.FX.single_run: 
                ...
                if err != 0: return -2, _('Error reloading Feeds after successfull update: %a'), err

            if restoring:
                err = self.FX.update_stats()
                if err != 0: return -2, _('Error updating DB stats after successfull update: %a'), err
            ...
            if len(self.to_update) > 1:
                ...
                return 0, f'{stype} {_("%a updated successfuly!")}', self.name()
            else:
                for f in self.to_update:
                    ...
                    if f == 'parent_id' and self.vals[f] not in (None, 0): 
                        ...
                        return 0, f'{stype} {_("%a assigned to")} {self.parent_feed.name(id=True)}', self.name(id=True)

                    elif f == 'parent_id' and self.vals[f] in (None, 0): 
                        self.FX.log(False, f'{stype} {self.name(id=True)} detached from category')
                        return 0, f'{stype} {_("%a detached from category")}', self.name(id=True)

                    elif f == 'error' and self.vals[f] in (None,0):
                        self.FX.log(False, f'{stype} {self.name(id=True)} marked as healthy')
                        return 0, f'{stype} {_("%a marked as healthy")}', self.name(id=True)
                        
                    elif f == 'deleted' and self.vals[f] in (None,0):
                        self.FX.log(False, f'{stype} {self.name(id=True)} restored')
                        return 0, f'{stype} {_("%a restored")}', self.name(id=True)

                    elif f == 'fetch' and self.vals[f] in (None,0):
                        self.FX.log(False, f'Fetching disabled for {self.name(id=True)}')
                        return 0, _('Fetching disabled for %a'), self.name(id=True)
                    elif f == 'fetch' and self.vals[f] == 1:
                        self.FX.log(False, f'Fetching enabled for {self.name(id=True)}')
                        return 0, _('Fetching enabled for %a'), self.name(id=True)


                    elif f == 'auth':
                        self.FX.log(False, f'Authentication method changed for {self.name(id=True)}')
                        return 0, _('Authentication method changed for %a'), self.name(id=True)
                    elif f == 'passwd':
                        self.FX.log(False, f'Password changed for {self.name(id=True)}')
                        return 0, _('Password changed for %a'), self.name(id=True)
                    elif f == 'login':
                        self.FX.log(False, f'Login changed for {self.name(id=True)}')
                        return 0, _('Login changed for %a'), self.name(id=True)
                    elif f == 'domain':
                        self.FX.log(False, f'Domain (auth) changed for {self.name(id=True)}')
                        return 0, _('Domain (auth) changed for %a'), self.name(id=True)

                    else:
                        self.FX.log(False, f'{stype} {self.name(id=True)} updated: {f} -> {self.vals.get(f,"<NULL>")}')
                        return 0, f'{stype} {_("%a updated")}:  {f} -> {self.vals.get(f,_("<NULL>"))}', self.name(id=True)
    ...
    def r_delete(self, **kargs):
        ...
        if deleted == 1:
            ...
            try: self.FX.ixer_db.delete_document(f'FEED_ID {self.vals["id"]}')
            ...
            if err == 0:
                ...
                if icon is not None and icon.startswith(os.path.join(self.FX.icon_path,'feed_')) and os.path.isfile(icon): os.remove(icon)
        ...
        if self.FX.rowcount > 0:
            ...
            if not self.FX.single_run: 
                ...
                if err != 0: return -2, _('Error reloading data after successfull delete: %a'), err
            ...
            if deleted == 1:
                ...
                return 0, f'{stype} {_("%a deleted permanently (with entries and rules)")}', f'{self.name()} ({self.vals["id"]})'
            else:
                self.vals['deleted'] = 1
                self.FX.log(False, f'{stype} {self.name()} ({self.vals["id"]}) deleted')
                return 0, f'{stype} {_("%a deleted")}', f'{self.name()} ({self.vals["id"]})'
        else:
            return 0, _('Nothing done')
    ...
    def r_add(self, **kargs):
        ...
        self.vals['id'] = None
        self.vals['display_order'] = len(self.FX.MC.feeds)
        ...
        if not self.FX.single_run: 
            err = self.FX.load_feeds()
            if err != 0: return -2, _('Error reloading Feeds after successfull add: %a'), err

        if self.vals['is_category'] == 1: stype = _('Category')
        else: stype = _('Channel')

        self.FX.log(False, f'{stype} {self.name()} ({self.vals["id"]}) added')
        return 0, f'{stype} {_("%a added successfully")}', f'{self.name()} ({self.vals["id"]})'
    ...
    def g_add_from_url(self, **kargs):
        ...
        for f in self.FX.MC.feeds:
            if f[self.get_index('url')] == self.vals['url'] and f[self.get_index('deleted')] != 1:
                yield -7, _('Channel with this URL already exists (%a)'), f'{f[self.get_index("name")]} ({f[self.get_index("id")]})'
                return -7
        ...
        if err == 0:
            ...
            yield -2, _('Error while reloading Feeds for fetching: %a'), err
            return -2 
    ...
    def r_update_feed_order(self, **kargs):
        """ Updates feed order to DB according current loaded list """
        update_list = []
        for i,f in enumerate(self.FX.MC.feeds):
            id = f[self.get_index('id')]
            update_list.append({'display_order':i, 'id':id})
        err = self.FX.run_sql_lock('update feeds set display_order = :display_order where id = :id', update_list, many=True)
        if err != 0: return -2, _('DB error while updating feed list order: %a'), err
        return 0


    def order_insert(self, target:int, **kargs): self.FX.MC.ret_status = cli_msg(self.r_order_insert(target, **kargs))
    def r_order_insert(self, target:int, **kargs):
        """ Insert (display order) before target, or assign to category """
        target = FeedContainer(self.FX, id=target)
        if not target.exists: return -8, _('Target by id %a not found'), target
        if not self.exists: return -8, _('Channel/Category %a not found'), self.vals.get('id')
        ...
        if (self.vals['is_category'] == 1 and target['is_category'] == 1) or (self.vals['is_category'] != 1 and target['is_category'] != 1):
            ...
            try: self.FX.MC.feeds.insert(target_pos, self.FX.MC.feeds[source_pos])
            except (ValueError, IndexError) as e: return -7, _('Error insering Channel/Category: %a'), e

            for i,f in enumerate(self.FX.MC.feeds):
                if f[self.get_index('id')] == self.vals['id'] and i != target_pos:
                    try: del self.FX.MC.feeds[i]
                    except (ValueError, IndexError) as e:return -7, _('Error insering Channel/Category: %a'), e
                    break

            err = self.r_update_feed_order()
            if err != 0: return err
            ...
            if with_cat:
                if (self.vals['is_category'] != 1 and target['is_category'] != 1) and (self.vals['parent_id'] != target['parent_id']) and (target['parent_id'] is not None):
                    err = self.r_update({'parent_id': target.get('parent_id')})
                    if err != 0: return err

        elif (self.vals['is_category'] != 1 and target['is_category'] == 1):
            err = self.r_update({'parent_id': target.get('id')})
            if err != 0: return err
        
        else:
            return -8, _('Nothing to do')

        if not self.FX.single_run:
            err = self.FX.load_feeds()
            if err != 0: return -2, _('Error reloading Feeds after successfull operation: %a'), err
